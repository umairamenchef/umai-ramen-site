...
- **Carte d'été mobile zoom** (commit 688a825 → superseded by 573a128): first shipped a tap-to-toggle native-zoom lightbox; EK found it not mobile-adapted. **Replaced with a real gesture viewer** (`react-zoom-pan-pinch` ^3.7.0, React 19 OK): `CarteEteViewer` full-screen — pinch-to-zoom, drag-to-pan, double-tap, +/−/reset buttons, ‹/› page nav, Esc/✕ close. i18n zoomHint/close FR/EN/DE. Lib bundled + live ✓ (touch gestures need a real device to fully validate). NB: new dep → Docker `npm install` rebuild; committed package.json + package-lock.json.
