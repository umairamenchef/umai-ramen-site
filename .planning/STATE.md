...
- **Carte d'été mobile zoom** (commit 688a825): `/carte-ete` pages are dense 2-col A-format → added `CarteEteViewer` client lightbox: tap a page → full-screen, tap toggles fit ↔ full-res (pan by scroll), native pinch-zoom on top, Esc/✕/backdrop closes. i18n zoomHint/close FR/EN/DE. Viewport already allows pinch (no user-scalable=no). Live ✓.
