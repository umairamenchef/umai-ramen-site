...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'meta.politiqueCookies' });
  return {
    metadataBase: new URL(BASE_URL),
    title: t('title'),
    description: t('description'),
    alternates: buildAlternates(locale, '/politique-cookies'),
...
export default async function PolitiqueCookiesPage({ params }: Props) {
  ...
  const sections = ['what', 'used', 'manage', 'google'] as const;
